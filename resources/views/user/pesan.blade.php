...
<a href="{{ route('go_shopping') }}">
  <button type="button"
    class="ml-11 w-full flex items-center justify-center w-1/2 px-2 py-2  transition-colors duration-200 bg-white border rounded-full border-black gap-x-2 sm:w-auto dark:hover:bg-gray-800 dark:bg-gray-900 hover:bg-gray-100 dark:text-gray-200 dark:border-gray-700">
    <svg class="w-4 h-4 rtl:rotate-180" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20"
...
@if($folder_name === 'banner')
  ...
  <form action="{{ route('order.store') }}" method="POST">
    ...
    <div class="mb-8">
    <input type="text" id="field1" name="judul" placeholder="Judul Banner"
    ...
    <input type="text" id="nama{{ $i }}" name="nama{{ $i }}" placeholder="Nama({{ $i }})"
    class="mt-1 p-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
    </div>
    <div class="w-full md:w-1/2 pl-2">
    <input type="text" id="deskripsi{{ $i }}" name="deskripsi{{ $i }}" placeholder="Deskripsi({{ $i }})"
    class="mt-1 p-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
    </div>
    </div>
  @endfor
    ...
    <a href="{{ route('about_us') }}"><button type="button"
      class="bg-gray-700 hover:bg-blue-700 text-white py-2 px-4 rounded-full">Checkout</button></a>
    <a href="{{ route('cart') }}"><button type="submit"
      class="font-sans block mt-4 lg:inline-block lg:mt-0 lg:ml-6 align-middle text-black hover:text-gray-700 relative flex"></a>
...
@elseif($folder_name === 'photocard')
  ...
  <form action="{{ route('order.store') }}" method="POST">
    @csrf <!-- Menambahkan CSRF token -->
    <div class="mb-8">
    <input type="text" id="field1" name="judul" placeholder="Judul Photocard"
      class="mt-1 p-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
    </div>
    <input type="hidden" name="img" value="{{ $photo->image_url }}">
    <input type="hidden" name="harga" value="{{ 35000 }}">
